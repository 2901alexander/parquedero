...
def CuadreCaja(ingrVhic):
    ...
    # Impresión de los valores de cuadratura de caja
...
def capturar_informacion(regist):
    ...
    while True:
        ...
        if re.match(regex_auto, placa) or re.match(regex_moto, placa):
    ...
    while True:
        ...
        if tipo_vehiculo in ['a', 'm']:
            if (tipo_vehiculo == 'a' and re.match(regex_auto, placa)) or (tipo_vehiculo == 'm' and re.match(regex_moto, placa)):
                vehiculosregist.append(tipo_vehiculo)
                if tipo_vehiculo == 'a':
                    print("Registro de automóvil exitoso.")
                else:
...
def mostrartarifas(valores_tarifas):
    ...
    print(f"Valor por minuto Automóviles: {valores_tarifas[0]}")
